...
v32-maraton kopyasi (2026-08-11): gen-200 artefaktiyla AYNI, tek fark
ic butce kirpmasi 50 -> 320 sn (orijinal 50 sn'lik dilim rejimine gore
evrilmisti; 300 sn'lik maraton butcesini kullanabilsin diye).
"""
...
ORBIT_CAND = 64           # candidate bases scored per orbit pick
ORBIT_ROUNDS = 12
...
class TIndex:
    ...
    def ids_with(self, rest, e):
        ...
        if got is None:
            ...
            # The live working set at v=32,k=8 is ~(#blocks * k) rests x v
            # elements ~= 100k keys, so a 200k cap was firing constantly and
            # destroying the hit rate.  Give it real headroom.
            if len(wc) >= 1500000:
                wc.clear()
            wc[key] = got
        return got
...
def orbit_gain(orbit, tix, cov, stamp, tag):
    """New t-subsets covered by the whole orbit, counting duplicates once.
    Uses a reusable stamp array instead of building a Python set (the set
    version allocated up to v*C(k,t) hashed ints per candidate)."""
    g = 0
    for b in orbit:
        for j in tix.block_ids(b):
            if cov[j] == 0 and stamp[j] != tag:
                stamp[j] = tag
                g += 1
    return g
    ...
    while remaining > 0 and len(blocks) < limit:
        ...
        for _ in range(ORBIT_CAND):
            ...
            if not orb:
                continue
            tag += 1
            g = orbit_gain(orb, tix, cov, stamp, tag)
            if g == 0:
                continue
            eff = g / float(len(orb))
            if eff > best_eff:
        ...
        if best_orb is None:
            break
        # Add the orbit block by block, but STOP as soon as the marginal
        # block stops paying.  The threshold is now tied to the ACTUAL
        # payoff of this orbit (a fraction of its average per-block gain)
        # instead of the meaningless constant `t`; that constant let
        # near-worthless blocks in during the late, expensive rounds.
        avg = best_gain / float(len(best_orb))
        min_pay = max(1, int(avg * 0.34))
        added_any = False
        for b in best_orb:
            if len(blocks) >= limit:
                break
...
class FixedSize:
    ...
    def __init__(self, v, k, t, tix, blocks, rng):
        ...
        # sorted-tuple mirror, kept in sync: removes sorted() from the
        # inner loop entirely
        self.btup = [tuple(sorted(b)) for b in self.blocks]
        self.cov = [0] * tix.n
        for bt in self.btup:
    ...
    def _rest(self, bi, out_e):
        """Sorted tuple of block bi minus out_e, WITHOUT set-difference and
        WITHOUT sorting: btup is maintained sorted, so one linear scan is
        enough.  sorted(set-{x}) was allocating a set + a list + a tuple on
        every single candidate evaluation; this is the hottest path in the
        whole solver."""
        bt = self.btup[bi]
        return tuple(e for e in bt if e != out_e)

    def _swap_ids(self, bi, out_e, in_e):
        rest = self._rest(bi, out_e)
        return (self.tix.ids_with(rest, out_e),
                self.tix.ids_with(rest, in_e))

    def _delta(self, rem, add):
        """rem and add are DISJOINT: every id in rem contains out_e and
        every id in add contains in_e, and out_e != in_e is guaranteed by
        the callers.  So no overlap bookkeeping is needed at all -- this
        removes two set() constructions per candidate evaluation, which
    ...
    def _apply(self, bi, out_e, in_e, rem, add, d):
        ...
        blk.discard(out_e)
        blk.add(in_e)
        # keep the sorted mirror in sync (insertion into a sorted tuple)
        bt = self.btup[bi]
        lst = [e for e in bt if e != out_e]
        lo, hi = 0, len(lst)
        while lo < hi:
            mid = (lo + hi) >> 1
            if lst[mid] < in_e:
        ...
        while self.cost > 0:
            ...
            for bi in cands:
                ...
                if len(miss) != 1:
                    continue
                in_e = next(iter(miss))
                rest_full = self.btup[bi]
                for out_e in rest_full:
                    if out_e in tset:
                        continue
                    rem, add = self._swap_ids(bi, out_e, in_e)
                    ...
                    if best_d is None or d < best_d or (
                        ...
                        if d < -1:
                            # a move that covers the target AND fixes more
                            # than it breaks: take it, stop scanning
                            break
...
def solve(v, k, t, budget, seed=0, on_improve=None):
    ...
    # Wall clock is only the HARD stop here; the phase is sized by work
    # (one grow per block, and a greedy covering needs O(n/C(k,t)) blocks).
    ...
    # Budget by ROUNDS of orbit-set construction (work counter), capped by
    # the global hard deadline.
    orb_dl = min(deadline - 0.4,
                 time.perf_counter() + (deadline - time.perf_counter()) * 0.30)
    rounds = 0
    ...
    # A fixed small NUMBER of restarts (work counter), not a time slice:
    # restarts have sharply diminishing returns versus the endgame.
    r_dl = min(deadline - 1.0,
               time.perf_counter() + (deadline - time.perf_counter()) * 0.08)
    restarts = 0
    while time.perf_counter() < r_dl and restarts < 3:
        restarts += 1
        blocks = greedy_cover(v, k, t, tix, set(range(tix.n)), rng,
                              deadline=r_dl)
        cov = build_cov(blocks, tix)
    ...
    while time.perf_counter() < deadline - 0.05 and len(best) > 1:
        ...
        # Not solved, but if the incumbent snapshot got very close we keep
        # hammering the SAME target rather than paying for ruin-recreate.
        if fs.best_cost <= max(1, k):
            fs.blocks = [set(b) for b in fs.best_snapshot]
            fs.btup = list(fs.best_snapshot)
            fs.cov = [0] * tix.n
            for bt in fs.btup:
                for j in tix.block_ids(bt):
                    fs.cov[j] += 1
            fs.uncov = {j for j, c in enumerate(fs.cov) if c == 0}
            fs.cost = len(fs.uncov)
            fs.where = [set() for _ in range(v)]
            for bi, b in enumerate(fs.blocks):
                for e in b:
                    fs.where[e].add(bi)
            for _ in range(2):
                fs._random_kick()
        ...
        if len(nb) < len(best):
            ...
            # Perturb IN PLACE: rebuilding FixedSize rescans all C(v,t)
            # ids and the incidence lists, which at v=32,t=4 costs more
            # than thousands of actual search moves.
...
def main(argv=None):
    ...
    budget = max(1.0, min(budget, 320.0))
